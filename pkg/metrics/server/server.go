...
type Server interface {
	...
	WithLogger(logger logging.Logger) Server
}
...
func (s *defaultServer) WithLogger(logger logging.Logger) Server {
	s.mu.RLock()
	currentBindAddr := s.bindAddr
	s.mu.RUnlock()
	// Create a new instance of the defaultServer with the provided logger.
	newServer := &defaultServer{
		options:       s.options,
		metricsFilter: s.metricsFilter,
		mu:            sync.RWMutex{},
		bindAddr:      currentBindAddr,
		log:           logger.WithName("metrics-server"),
	}

	// Deep copy the ExtraHandlers map to avoid modifying the original
	if s.options.ExtraHandlers != nil {
		newServer.options.ExtraHandlers = make(map[string]http.Handler, len(s.options.ExtraHandlers))
		for path, handler := range s.options.ExtraHandlers {
			newServer.options.ExtraHandlers[path] = handler
		}
	}
...
func (s *defaultServer) createListener(ctx context.Context, log logging.Logger) (net.Listener, error) {
