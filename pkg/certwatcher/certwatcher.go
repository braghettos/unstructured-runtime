...
// Return a copy of the CertWatcher with a logger set.
func (cw *CertWatcher) SetLogger(logger logging.Logger) *CertWatcher {
	cw.log = logger.WithName("CertWatcher")
	return cw
}
...
func (cw *CertWatcher) updateCachedCertificate(cert *tls.Certificate, keyPEMBlock []byte) bool {
	...
	if cw.currentCert != nil &&
		...
		log.Debug("certificate already cached")
		return false
	}
	cw.currentCert = cert
...
func (cw *CertWatcher) handleEvent(event fsnotify.Event) {
	...
	log.Debug("certificate event", "event", event)
	if err := cw.ReadCertificate(); err != nil {
